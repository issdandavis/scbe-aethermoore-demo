...
export function mldsaSign(
  ...
  // Hash message
  const mu = shake256(
    new Uint8Array([...shake256(new Uint8Array([...rho, ...flattenT(t)]), 64), ...message]),
    64
  );
...
function encodeMLKEMCiphertext(
  u: number[][],
  v: number[],
  du: number,
  dv: number,
...
function decodeMLKEMCiphertext(
  ct: Uint8Array,
  du: number,
  dv: number,
  k: number
