...
export const TIER_THRESHOLDS: Record<
  GovernanceTier,
  { xp: number; name: string; description: string }
> = {
...
export class PollyPadManager {
  ...
  public addSketch(
    padId: string,
    sketch: Omit<PadSketch, 'id' | 'createdAt' | 'updatedAt'>
  ): PadSketch {
    const pad = this.pads.get(padId);
    if (!pad) throw new Error(`Pad ${padId} not found`);
  ...
  public addTool(
    padId: string,
    tool: Omit<PadTool, 'id' | 'createdAt' | 'usageCount' | 'lastUsed' | 'effectiveness'>
  ): PadTool {
    const pad = this.pads.get(padId);
    if (!pad) throw new Error(`Pad ${padId} not found`);
   ...
   */
  public demoteTier(padId: string, demotionReason: string): boolean {
    const pad = this.pads.get(padId);
    if (!pad) return false;
  ...
  public getPadStats(padId: string):
    ...
    const progress =
      (pad.experiencePoints - prevThreshold) / (pad.nextTierThreshold - prevThreshold);
