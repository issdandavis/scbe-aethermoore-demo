...
### GitLab cross-pond hook (when failures involve mirroring or lore)

If CI failures correlate with cross-pond work (GitLab mirroring, lore/docs repos), run a read-only pond flush before chasing logs:

```powershell
pwsh -File "C:\Users\issda\SCBE-AETHERMOORE\scripts\gitlab\pond_flush.ps1" -GitLabRepoUrl "https://gitlab.com/<group>/<project>.git" -CheckGhAuth
```

Then proceed with standard `gh pr checks` inspection.
