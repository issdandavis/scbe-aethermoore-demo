 ...
 * ```
 */
function enforcePolicy(validTongues, policy = 'standard') {
    const required = exports.POLICY_MATRIX[policy];
    // Standard policy: any valid signature
    if (required.length === 0) {
        if (validTongues.length === 0) {
            throw new types_1.PolicyError('No valid signatures found');
        }
        return true;
    }
 ...
 */
function checkPolicy(validTongues, policy = 'standard') {
    try {
