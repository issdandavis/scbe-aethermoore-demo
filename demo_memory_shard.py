...
class SimpleGovernance:
    ...
    def make_decision(
        self, agent: str, topic: str, context: str = "internal"
    ) -> GovernanceResult:
...
class MemoryShardSystem:
    ...
    def retrieve_memory(
        ...
        if gov_result.decision == GovernanceDecision.DENY:
            ...
            return RetrievalResult(
                success=False, plaintext=None, governance=gov_result, trace=trace
            )
        ...
        try:
            ...
            return RetrievalResult(
                success=True, plaintext=plaintext, governance=gov_result, trace=trace
            )
...
def run_demo(
    ...
    context_map = {"normal": "internal", "elevated": "external", "high": "untrusted"}
...
def main():
    ...
    parser.add_argument(
        "--memory", "-m", default="Hello, AETHERMOORE!", help="Memory content to seal"
    )
    parser.add_argument("--agent", "-a", default="ash", help="Agent identifier")
    parser.add_argument("--topic", "-t", default="aethermoore", help="Topic/category")
    parser.add_argument(
        "--risk",
        "-r",
